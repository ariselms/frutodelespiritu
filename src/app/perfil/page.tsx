							...
							{user?.role === "admin" &&
								UserNavigationWithAuth.map(
									(item: NavigationItemTypeWithAuth) => (
										<div
											key={item.name}
											className="flex flex-col justify-start rounded-lg p-4 bg-slate-50 dark:bg-gray-900/50 border border-slate-100 dark:border-gray-600">
											...
											<Link
												href={item.href}
												className="flex items-center justify-center p-4 text-sm text-white font-bold uppercase dark:text-gray-50 rounded-lg cursor-pointer bg-slate-700 hover:bg-slate-800 border dark:bg-gray-900 dark:hover:bg-gray-800 dark:border-gray-600 focus:ring-4 focus:ring-slate-300  dark:focus:ring-gray-800 transition-all mt-auto">
												Administra
											</Link>
										</div>
									)
