	...
	const columns: ColumnDef<ArticleType>[] = [
		{
			...
			cell: ({ row, getValue }) => (
				<Link
					className="text-slate-600 dark:text-white underline hover:text-slate-800 dark:hover:text-gray-300"
			...
			<article className="container mx-auto px-2 text-black dark:text-white py-8">
				...
				<div className="overflow-x-auto">
					{userLectures.length > 0 ? (
						<>
							<table className="min-w-full border border-slate-200 dark:border-gray-600 border-collapse">
								<thead>
									...
									{table.getRowModel().rows.map((row) => (
										<tr
											...
											{row.getVisibleCells().map((cell) => (
												<td
													key={cell.id}
													className="border border-slate-200 dark:border-gray-600 px-4 py-2">
													{flexRender(
